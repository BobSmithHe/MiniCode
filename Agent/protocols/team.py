"""Team system — persistent teammate threads with idle-poll and auto-claim."""
...
from ..tools.task import (
    Task, list_tasks, load_task, claim_task, complete_task, can_start,
)
from ..tools.shell import run_bash as _shell_bash
from ..tools.file import run_read as _file_read, run_write as _file_write
from .messaging import BUS
from .approval import new_request_id, pending_requests, ProtocolState
...
def spawn_teammate_thread(name: str, role: str, prompt: str) -> str:
    ...
    def run():
        ...
        def _run_bash(command: str) -> str:
            return _shell_bash(command, cwd=_wt_cwd())

        def _run_read(path: str) -> str:
            return _file_read(path, cwd=_wt_cwd())

        def _run_write(path: str, content: str) -> str:
            return _file_write(path, content, cwd=_wt_cwd())
        ...
        while True:
            ...
            for _ in range(10):
                ...
                for msg in inbox:
                    stopped = handle_inbox_message(msg, messages)
                    if stopped:
